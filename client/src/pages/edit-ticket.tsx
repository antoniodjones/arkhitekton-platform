...
function EditTicketContent() {
  ...
  const handleSave = async () => {
    ...
    try {
      // Check for technical debt risk if this is an ADR being updated
      if (ticket.type === 'adr') {
        const riskIndicators = detectTechnicalDebtRisk({
          technicalDebtScore: ticket.metadata?.technicalDebtScore,
          riskAssessment: ticket.riskAssessment,
          decisionCategory: ticket.metadata?.decisionCategory,
          consequences: ticket.metadata?.consequences,
          maintenanceImpact: ticket.metadata?.maintenanceImpact,
          operationalImpact: ticket.metadata?.operationalImpact,
          affectedSystems: ticket.metadata?.affectedSystems,
          context: ticket.metadata?.context,
          decision: ticket.metadata?.decision,
          title: ticket.title
        });
  ...
  const handleCreateDebtTicket = (autoPopulate: boolean) => {
    if (!debtRiskIndicators || !ticket) return;

    const debtTicketData = {
      type: 'technical_debt',
      title: autoPopulate ? debtRiskIndicators.autoGeneratedTitle : '',
      description: autoPopulate ? 'Technical debt ticket auto-generated from updated ADR' : '',
      ...
      reviewers: [],
      watchers: [],
      labels: autoPopulate ? ['auto-generated', 'adr-derived', 'adr-update'] : [],
      ...
      debtScore: Math.floor(debtRiskIndicators.riskScore / 10),
      ...
      businessImpact: autoPopulate ? 'Impact analysis needed based on updated ADR consequences' : ''
    };
    ...
    if (autoPopulate) {
      ...
      // Update ADR to link to the new debt ticket
      const updatedTicket = { ...ticket };
      updatedTicket.metadata = updatedTicket.metadata || {};
      updatedTicket.metadata.linkedObjects = updatedTicket.metadata.linkedObjects || [];
      updatedTicket.metadata.linkedObjects.push({
      ...
      localStorage.setItem(`ticket-${updatedTicket.id}`, JSON.stringify(updatedTicket));
  ...
  return (
    <div className="h-full overflow-hidden flex flex-col">
      ...
      <div className="flex-1 overflow-auto">
        ...
        {showDebtPrompt && debtRiskIndicators && ticket && (
          <TechnicalDebtPrompt
